...
function AccountCard({ account, selected, onSelect, isCurrent: propIsCurrent, isRefreshing, onRefresh, onViewDetails, onExport, onDelete, onUpdateLabel, onViewError }: AccountCardProps) {
    ...
    const displayModels = useMemo(() => {
        ...
        const accountModels = account.quota?.models?.map(m => {
            // ：DEFAULT_MODELS  shortLabel，
            //  DEFAULT_MODELS  MODEL_CONFIG ， shortLabel
            // ， MODEL_CONFIG 
    ...
    return (
        <div className={cn(
            "flex flex-col p-3 rounded-xl transition-all duration-300 relative overflow-hidden group",
            isCurrent
                ? "bg-primary/5 border border-primary/20 dark:bg-primary/10 dark:border-primary/30 shadow-md scale-[1.01]"
                : "glass-panel glass-panel-hover",
            (isRefreshing || isDisabled) && "opacity-60"
        )}>
            {/*  */}
            {isCurrent && <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-transparent pointer-events-none" />}

            {/* Header: Checkbox + Email + Badges */}
            <div className="flex-none flex items-start gap-2 mb-2 relative z-10">
                <input
                    type="checkbox"
            ...
            <div className="flex-none flex items-center justify-center pt-2 pb-1 border-t border-gray-100 dark:border-base-200">
                ...
                {isEditingLabel && (
                    <div className="absolute inset-0 bg-white/95 dark:bg-base-100/95 rounded-xl z-10 flex items-center justify-center p-4">
                        <div className="flex items-center gap-2 w-full max-w-xs">
                            ...
                            />
                            <button
                                className="p-1 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-all"
                            ...
                            <button
                                className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-all"
                ...
                <div className="flex flex-wrap items-center justify-center gap-1 w-full">
                    <button
                        className="p-1 text-gray-400 hover:text-sky-600 dark:hover:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-900/30 rounded-lg transition-all"
                    ...
                    <button
                        className={`p-1 rounded-lg transition-all ${isRefreshing
