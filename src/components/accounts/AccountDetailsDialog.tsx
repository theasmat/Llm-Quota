    ...
    return createPortal(
        <div className="modal modal-open z-[100]">
            ...
            <div className="modal-box relative max-w-3xl bg-white dark:bg-base-100 shadow-2xl rounded-2xl p-0 overflow-hidden">
                ...
                {/* Quota Details */}
                <div className="p-6">
                    <h4 className="font-semibold text-gray-900 dark:text-base-content mb-4 flex items-center gap-2">
                        <Bot size={18} className="text-blue-500" />
                        {t('accounts.details.models')}
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {(() => {
                            if (!account.quota?.models || account.quota.models.length === 0) return null;
                            const mappedModels = account.quota.models.map(m => ({ id: m.name, data: m }));
                            const sortedModels = sortModels(mappedModels);
                            return sortedModels.map((model, idx) => (
                                <div key={idx} className="p-4 rounded-xl border border-gray-100 dark:border-base-300 bg-gray-50/50 dark:bg-base-200/50 hover:bg-gray-50 dark:hover:bg-base-200 transition-colors group">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex flex-col">
                                            <span className="font-bold text-gray-900 dark:text-base-content group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{MODEL_CONFIG[model.id]?.label || model.id}</span>
                                            <span className="text-[10px] text-gray-400 font-mono mt-0.5">{model.id}</span>
                                        </div>
                                        <span
                                            className={`text-xs font-bold px-2 py-0.5 rounded-md ${model.data.percentage >= 50 ? 'bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                                model.data.percentage >= 20 ? 'bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' :
                                                    'bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                                                }`}
                                        >
                                            {model.data.percentage}%
                                        </span>
                                    </div>
                                    ...
                                    <div className="h-1.5 w-full bg-gray-100 dark:bg-base-200 rounded-full overflow-hidden mb-3">
                                        <div
                                            className={`h-full rounded-full transition-all duration-500 ${model.data.percentage >= 50 ? 'bg-emerald-500' :
                                                model.data.percentage >= 20 ? 'bg-orange-400' :
                                                    'bg-red-500'
                                                }`}
                                            style={{ width: `${model.data.percentage}%` }}
                                        ></div>
                                    </div>

                                    <div className="flex items-center gap-1.5 text-[10px] text-gray-400 dark:text-gray-500 font-mono">
                                        <Clock size={10} />
                                        <span>{t('accounts.reset_time')}: {formatDate(model.data.reset_time) || t('common.unknown')}</span>
                                    </div>
                                </div>
                            ));
                        })() || (
